...
class ContourLabeler(object):
    ...
    def clabel(self, *args, **kwargs):
        ...
        transform : :class:`Transform`
            A transform to specify the coordinate frame of the x,y positions
            in an iterable *manual*.

            If *None*, data coordinate will be used for the x,y positions. If
            *manual* is not an iterable object, this is ignored.
        ...
        if transform is not None:
            if (not isinstance(transform, mtransforms.Transform)):
                raise TypeError(
                    "Argument 'transform' to clabel is not a Transform")
        ...
        if cbook.iterable(self.labelManual):
            for x, y in self.labelManual:
                ...
                if isinstance(y, list):
                    intersects = self.get_contour_intersects(
                        x, fixed_coord=0, transform=transform)
                    x_or_y = y
                elif isinstance(x, list):
                    intersects = self.get_contour_intersects(
                        y, fixed_coord=1, transform=transform)
                    x_or_y = x

                if intersects is not None:
                    # skip this placement if no intersection
                    if intersects.size == 0:
                        continue

                    # clamp argument index to valid range
                    if len(x_or_y) < 1:
                        i = 0
                    elif x_or_y[0] >= intersects.size:
                        i = intersects.size - 1
                    elif x_or_y[0] < -intersects.size:
                        i = -intersects.size
                    else:
                    ...
                    # set (x, y) for label positioning
                    x = intersects[i, 0]
                    y = intersects[i, 1]

                self.add_label_near(x, y, inline,
                                    inline_spacing, transform)
    ...
    def get_contour_intersects(self, x_or_y, fixed_coord=0, indices=None,
                               transform=None):

        """
        Finds all intersection points at x = *x_or_y* if *fixed_coord* is 0 or
        at y = *x_or_y* if *fixed_coord* is 1.

        Returns a numpy array of [x, y] intersects, sorted in increasing order
        of the un-fixed coordinate.
        ...
        Optional keyword arguments:
          ...
          *transform*:
            The coordinate frame of *x_or_y*.
        """
        ...
        if transform:
            if (fixed_coord == 0):
                x_or_y, __ = transform.transform_point((x_or_y, 0))
            else:
                fixed_coord = 1
                __, x_or_y = transform.transform_point((0, x_or_y))

        if indices is None:
            indices = list(range(len(self.levels)))

        # determine maximum number of possible intersects to
        # initialize intersects array size
        max_count = 0
        for icon in indices:
            con = self.collections[icon]
            paths = con.get_paths()
            max_count += 2 * len(paths)

        all_intersects = np.empty([max_count, 2])
        ...
        for icon in indices:
            ...
            for linepath in paths:
                lc = linepath.vertices

                # transfer all data points to screen coordinates
                lc = trans.transform(lc)

                # pick a tolerable intersect if available
                ipath_intersects = np.where(
                                    np.isclose(lc[:, fixed_coord], x_or_y))[0]
                path_intersects = lc[ipath_intersects, :]
                if (path_intersects.size > 0):
                    interc1 = path_intersects[0, :]

                    # add it to the final intersects array
                    all_intersects[real_count, :] = interc1
                    real_count += 1

                    # check for an additional (different) intersect to add
                    iextra_intersects = np.where(
                                            np.invert(
                                                np.all(
                                                    np.isclose(
                                                        interc1,
                                                        path_intersects),
                                                    axis=1)))[0]
                    if (iextra_intersects.size > 0):
                        interc2 = path_intersects[iextra_intersects[0]]
                        all_intersects[real_count, :] = interc2
                        real_count += 1
        a = all_intersects[0:real_count, :]
        return transform.inverted().transform(a[a[:, fixed_coord-1].argsort()])
